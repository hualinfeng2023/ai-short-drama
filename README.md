...
项目首次创建时，若用户未手填短剧名称，服务端会根据故事想法、题材、风格、独立叙事定位和市场生成名称，并可在方舟文本服务不可用时使用明确标注的本地命名回退。Brief 编辑页的“优化名称”会携带当前尚未保存的完整 Brief，要求文本模型在内部比较多个候选后返回一条与原名不同、锚定核心人物／冲突／意象且具有传播力的新名称；原名、泛化标题和与故事缺少语义锚点的结果会被拒绝，服务不可用时明确失败，不用本地规则随意改名。候选名称只有在用户保存 Brief 新版本后才成为项目事实。
