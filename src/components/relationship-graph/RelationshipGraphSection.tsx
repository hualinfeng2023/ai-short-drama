  ...
  return (
    <section className="story-section relationship-editor" aria-labelledby="relationship-editor-title" id="relationship-review">
      <div className="relationship-editor__heading">
        ...
        <div className="relationship-editor__version"><label>关系版本<select aria-label="选择关系版本" onChange={(event) => switchVersion(event.target.value)} value={selectedGraph.id}>{versions.map((version) => <option key={version.id} value={version.id}>第 {version.version} 版 · {version.status === 'DRAFT' ? '草稿' : version.status === 'READY_FOR_REVIEW' ? '待审核' : version.status === 'APPROVED' ? '已批准' : '历史版本'}</option>)}</select></label><StatusBadge status={selectedGraph.status} />{comparisonBase() ? <Button disabled={busy || draftState.dirty} onClick={() => void run(openDiff)} size="sm" variant="secondary"><GitCompare size={15} />比较版本</Button> : null}</div>
      </div>
      ...
      <div className="relationship-editor__workspace">
        ...
        <aside className="relationship-inspector" aria-label={selectedCharacter ? '角色信息' : '关系属性'}>
          {selectedCharacter ? <>
            ...
            <label>关系方向<select disabled={!canEditSelected} onChange={(event) => updateEdge((edge) => { edge.directionality = event.target.value as RelationshipEdgeRecord['directionality'] })} value={selectedEdge.directionality}><option value="BIDIRECTIONAL">双向关系</option><option value="DIRECTED">由前者指向后者</option></select></label>
            ...
            {selectedEdge.relationshipTypes.includes('FAMILY') ? <fieldset className="relationship-kinship-fields" disabled={!canEditSelected}><legend>血缘与成长环境</legend><label>亲属来源<select onChange={(event) => updateEdge((edge) => { edge.familyKinship = { ...(edge.familyKinship ?? { sharedUpbringing: 'UNKNOWN' }), relationType: event.target.value as FamilyRelationType } })} value={selectedEdge.familyKinship?.relationType ?? 'UNSPECIFIED'}>{(Object.keys(FAMILY_RELATION_LABELS) as FamilyRelationType[]).map((type) => <option key={type} value={type}>{FAMILY_RELATION_LABELS[type]}</option>)}</select></label><label>共同成长环境<select onChange={(event) => updateEdge((edge) => { edge.familyKinship = { ...(edge.familyKinship ?? { relationType: 'UNSPECIFIED' }), sharedUpbringing: event.target.value as SharedUpbringing } })} value={selectedEdge.familyKinship?.sharedUpbringing ?? 'UNKNOWN'}>{(Object.keys(SHARED_UPBRINGING_LABELS) as SharedUpbringing[]).map((type) => <option key={type} value={type}>{SHARED_UPBRINGING_LABELS[type]}</option>)}</select></label><label>成长经历说明<textarea onChange={(event) => updateEdge((edge) => { edge.familyKinship = { ...(edge.familyKinship ?? { relationType: 'UNSPECIFIED', sharedUpbringing: 'UNKNOWN' }), upbringingContext: event.target.value } })} placeholder="例如：从小共同生活，但因家庭冲突形成不同的情绪表达方式" rows={3} value={selectedEdge.familyKinship?.upbringingContext ?? ''} /></label><small>只有明确的亲生血缘会生成容貌相似约束；养亲、继亲和姻亲不会自动添加。</small></fieldset> : null}
