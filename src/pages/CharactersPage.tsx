...
const CUSTOM_SKIN_TONE_VALUE = '__custom_skin_tone__'
const SKIN_TONE_UNSPECIFIED = '未指定'
...
const SKIN_TONE_OPTIONS = [
  ...
  '小麦色',
  '橄榄色',
  SKIN_TONE_UNSPECIFIED,
] as const

function normalizeSkinToneValue(value: string): string {
  const trimmed = value.trim()
  if (!trimmed || /^unspecified$/i.test(trimmed) || trimmed === '未指定') {
    return SKIN_TONE_UNSPECIFIED
  }
  return trimmed
}

function isKnownSkinToneOption(value: string): boolean {
  return (SKIN_TONE_OPTIONS as readonly string[]).includes(value)
}

const CUSTOM_HAIR_COLOR_VALUE = '__custom_hair_color__'
const HAIR_COLOR_UNSPECIFIED = '未指定'
...
function normalizeHairColorValue(value: string): string {
  const trimmed = value.trim()
  if (!trimmed || /^unspecified$/i.test(trimmed) || trimmed === '未指定') {
    return HAIR_COLOR_UNSPECIFIED
  }
  return trimmed
}

function isKnownHairColorOption(value: string): boolean {
  return (HAIR_COLOR_OPTIONS as readonly string[]).includes(value)
}
...
function buildImageDownloadName(characterName: string, label: string): string {
  const safeName = `${characterName}-${label}`.replace(/[\\/:*?"<>|]/g, '-').trim()
  return `${safeName || 'character-image'}.png`
}
...
function profileDraft(character: CharacterVisualRecord): ProfileDraft | null {
  ...
  return {
    ...
    hairColor: appearance.hair_color ?? appearance.hair_texture ?? '',
...
function characterCardDescription(
  character: CharacterVisualRecord,
  profile: CharacterVisualProfile | undefined,
  liveDraft: ProfileDraft | null,
): string {
  const fallback = character.visualBrief || '暂无角色描述'
  if (liveDraft) {
    return buildCharacterCardDescription({
      age: liveDraft.age,
      genderExpression: liveDraft.genderExpression,
      height: liveDraft.height,
      occupation: liveDraft.occupation,
      identifyingFeatures: liveDraft.identifyingFeatures,
      wardrobe: liveDraft.wardrobe,
      hairColor: liveDraft.hairColor,
      hairstyle: liveDraft.hairstyle,
  ...
  if (profile) {
    return buildCharacterCardDescription({
      ...
      hairColor: profile.appearanceFields.hair_color ?? profile.appearanceFields.hair_texture ?? '',
      hairstyle: profile.appearanceFields.hairstyle ?? '',
      fallbackSummary: fallback,
    })
  }
  return fallback
...
export function CharactersPage() {
  ...
  const [customSkinToneActive, setCustomSkinToneActive] = useState(false)
  const [customHairColorActive, setCustomHairColorActive] = useState(false)
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (!active) return
        setError(null)
        const generating = next?.characters.some((character) => (
          character.status === 'GENERATING'
          || character.batches.some((batch) => batch.status === 'GENERATING')
        )) ?? false
        timer = window.setTimeout(poll, generating ? 1000 : 3000)
      } catch (reason) {
  ...
  async function downloadCandidateImage() {
    if (!candidateImageViewer) return
    try {
      await downloadImageAsset(
        candidateImageViewer.assetUrl,
        buildImageDownloadName(candidateImageViewer.characterName, candidateImageViewer.variantLabel),
      )
    } catch {
  ...
  function edit(character: CharacterVisualRecord) {
    ...
    const skinTone = normalizeSkinToneValue(nextDraft?.skinTone ?? '')
    setCustomSkinToneActive(Boolean(skinTone) && !isKnownSkinToneOption(skinTone))
    const hairColor = normalizeHairColorValue(nextDraft?.hairColor ?? '')
    setCustomHairColorActive(Boolean(hairColor) && !isKnownHairColorOption(hairColor))
  }
  ...
  function closeVisualEditor() {
    ...
    setCustomSkinToneActive(false)
    setCustomHairColorActive(false)
  }

  function renderVisualEditorGrid() {
    if (!draft) return null
    const regionValue = normalizeRegionValue(draft.region)
    ...
    const heightValue = normalizeHeightValue(draft.height)
    ...
    const skinToneValue = normalizeSkinToneValue(draft.skinTone)
    const skinToneOptions = Array.from(new Set([
      ...(skinToneValue && !isKnownSkinToneOption(skinToneValue) && !customSkinToneActive
        ? [skinToneValue]
        : []),
      ...SKIN_TONE_OPTIONS,
    ]))
    const hairColorValue = normalizeHairColorValue(draft.hairColor)
    const hairColorOptions = Array.from(new Set([
      ...(hairColorValue && !isKnownHairColorOption(hairColorValue) && !customHairColorActive
        ? [hairColorValue]
        : []),
      ...HAIR_COLOR_OPTIONS,
    ]))
    return (
      <div className="character-visual-editor__grid" data-entity-kind={draft.entityKind}>
        ...
        <label className="is-human-only">肤色<SelectControl
          aria-label="肤色"
          onChange={(event) => {
            const nextValue = event.target.value
            const custom = nextValue === CUSTOM_SKIN_TONE_VALUE
            setCustomSkinToneActive(custom)
            setDraft({ ...draft, skinTone: custom ? '' : nextValue })
          }}
          searchable
          value={customSkinToneActive ? CUSTOM_SKIN_TONE_VALUE : skinToneValue}
        >
          {skinToneOptions.map((option) => (
            <option key={option} value={option}>{option}</option>
          ))}
          <option value={CUSTOM_SKIN_TONE_VALUE}>其他／自定义描述</option>
        </SelectControl>
        {customSkinToneActive ? (
          <input
            aria-label="自定义肤色"
            onChange={(event) => setDraft({ ...draft, skinTone: event.target.value })}
            placeholder="例如：自然偏暖、略深的小麦色"
        ...
        <label className="is-human-only">发色<SelectControl
          aria-label="发色"
          onChange={(event) => {
            const nextValue = event.target.value
            const custom = nextValue === CUSTOM_HAIR_COLOR_VALUE
            setCustomHairColorActive(custom)
            setDraft({ ...draft, hairColor: custom ? '' : nextValue })
          }}
          searchable
          value={customHairColorActive ? CUSTOM_HAIR_COLOR_VALUE : hairColorValue}
        >
          {hairColorOptions.map((option) => (
            <option key={option} value={option}>{option}</option>
          ))}
          <option value={CUSTOM_HAIR_COLOR_VALUE}>其他／自定义描述</option>
        </SelectControl>
        {customHairColorActive ? (
          <input
            aria-label="自定义发色"
            onChange={(event) => setDraft({ ...draft, hairColor: event.target.value })}
            placeholder="例如：浅棕带金、深黑偏冷"
  ...
  const editingCharacter = editingId
    ? workspace.characters.find((character) => character.id === editingId) ?? null
    : null
  ...
  return <div className="page page--characters page--character-visuals">
    ...
    <div className="character-page-body">
      ...
      const displayedCandidateBatchId = generatingCandidates
        ? (activeCandidateBatch?.id ?? newestCandidateBatch?.id)
        : latestCompletedBatchId
      const batchFirstOrdinal = resolveBatchFirstOrdinal(
        ...
        : character.status === 'LOCKED'
          ...
          : pendingIdentity
            ? pendingIdentity.status === 'READY_FOR_REVIEW' && !viewingReplacedIdentity
              ...
              : 3
            : 2
      const liveDraft = editingId === character.id && draft ? draft : null
      return <article className="character-visual-card" data-character-id={character.id} id={`character-${character.id}`} key={character.id} tabIndex={-1}>
        <header className="character-visual-card__header">
          <div><p className="eyebrow">{localizeCharacterRole(character.role)}</p><div className="character-visual-card__title-row"><h2>{character.name}</h2>{profile ? <span className="character-profile-version">第 {profile.version} 版</span> : null}</div><small>{characterCardDescription(character, profile, liveDraft)}</small></div>
          {profile ? <div className="character-visual-card__actions">
            <Button disabled={generating || busy !== null} onClick={() => edit(character)} size="sm" variant="secondary"><Pencil size={14} />调整设定</Button>
          </div> : null}
        </header>
        ...
        {profile && blockers.length > 0 ? <div className="character-profile-blockers" role="alert">
          <AlertTriangle size={16} />
          <div>
            <strong>设定待补充（{blockers.length} 项）</strong>
            <ul>{blockers.map((issue) => <li key={issue.code}>{issue.message}</li>)}</ul>
          </div>
          <Button disabled={generating || busy !== null} onClick={() => edit(character)} size="sm" variant="secondary"><Pencil size={14} />调整设定</Button>
        </div> : null}
    ...
    <Modal
      ...
      footer={editingCharacter ? <>
        <Button disabled={busy !== null} onClick={closeVisualEditor} variant="secondary">取消</Button>
        <Button disabled={busy !== null} onClick={() => void saveProfile(editingCharacter)}>{busy === `save-${editingCharacter.id}` ? <LoaderCircle className="spin" size={15} /> : <ShieldCheck size={15} />}保存并重新审核</Button>
      </> : null}
      onClose={closeVisualEditor}
      open={editingCharacter !== null && draft !== null}
      title="外貌、造型与表演表现"
    >
      {renderVisualEditorGrid()}
    </Modal>
    ...
    <Modal
      ...
      footer={<>
        {candidateImageViewer ? <Button onClick={() => void downloadCandidateImage()} variant="secondary"><Download size={15} />下载</Button> : null}
